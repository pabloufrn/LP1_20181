...
void graal::qsort(void* base, size_t num, size_t size, int (*compar)(const void*,const void*))
{
	...
	byte * dp = reinterpret_cast<byte*>(partition(base, blast, size, compar)); // divide point, elemento na posição final
	qsort(base, (dp-bbase)/size, size, compar);
	qsort(dp + size, (blast-(dp+size))/size, size, compar); // ordena posição depois do dp (dp->after) até o last ([last - dp->after] elementos)
}
